...
class AppConfig(object):
    ...
    def get_supported_schemas(self):
        ...
        for item in os.listdir(self.schema_dir):
            if item.endswith(".yaml"):
                app_schema_file = os.path.join(self.schema_dir, item)
                with open(app_schema_file, "r", encoding="utf-8") as f:
                    app_schema_yaml = yaml.safe_load(f.read())
                if app_schema_yaml.get("name"):
                    app_schemas.append(app_schema_yaml.get("name"))
        return app_schemas

    def get_schema_by_name(self, schema_name: str):
        schema_file_path = os.path.join(self.schema_dir, "{}.yaml".format(schema_name))
        if not os.path.exists(schema_file_path):
            return None
        with open(schema_file_path, "r", encoding="utf-8") as f:
            schema_yaml = yaml.safe_load(f.read())
        return schema_yaml

    def get_template_by_path(self, template_path: list):
        tmpl_file_path = os.path.join(self.tmpl_dir, *template_path)
    ...
    def copy_from_schema(self, schema_name: str, vault_path):
